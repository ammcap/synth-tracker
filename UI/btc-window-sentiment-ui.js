...
function summarizeMarket(market, trades, prices) {
    ...
    let totalCost = 0;
    let totalPayout = 0;
    let postCloseRedemptionNotional = 0;
    let postCloseRedemptionCount = 0;

    trades.forEach(trade => {
        const notional = Number(trade.valueUSDC != null ? trade.valueUSDC : (trade.price || 0) * (trade.size || 0));
        if (!Number.isFinite(notional) || notional <= 0) return;
        const side = (trade.side || '').toUpperCase();
        if (side === 'BUY') {
            totalCost += notional;
        } else if (side === 'SELL') {
            totalPayout += notional;
            if (trade.ts > window.end) {
                postCloseRedemptionNotional += notional;
                postCloseRedemptionCount += 1;
            }
        }
    });
    ...
    const pnlLabel = `${netProfit >= 0 ? '+' : ''}${netProfit.toFixed(2)}`;
    const roiLabel = roiPct != null ? `${roiPct >= 0 ? '+' : ''}${roiPct.toFixed(2)}%` : 'n/a';
    log.info(`[${market}] ${inWindow.length} trades | ${windowMinutes} min window | Bias ${tradeDirection} (${netBias.toFixed(2)}) | BTC ${btcDirection}${changePct != null ? ` (${changePct.toFixed(2)}%)` : ''} | PnL ${pnlLabel} (ROI ${roiLabel})`);
    ...
    return {
        ...
        totalCost,
        totalPayout,
        postCloseRedemptionNotional,
        postCloseRedemptionCount,
        netProfit,
        roiPct,
...
function writeCsv(rows, filePath) {
    ...
    const lines = rows.map(row => [
        ...
        row.postCloseRedemptionNotional.toFixed(2),
        row.postCloseRedemptionCount,
        row.netProfit.toFixed(2),
        row.roiPct != null ? row.roiPct.toFixed(4) : ''
    ].join(','));
...
const clientBootstrap = () => {
    ...
    const computeSymmetricRange = (values) => {
        ...
        if (!usable.length) return { min: -10, max: 10 };
        const maxAbs = Math.max(1, ...usable.map((v) => Math.abs(v)));
    ...
    const buildLedger = (windowData) => {
        const startMs = windowData.windowStartMs;
        const trades = (windowData.trades || []).slice().sort((a, b) => a.tsMs - b.tsMs);
        let balance = 0;
        return trades.map((trade) => {
            balance += trade.signedNotional;
            return {
                trade,
                offsetMinutes: (trade.tsMs - startMs) / 60000,
                balance,
                activity: trade.side === 'SELL' ? 'Redemption' : 'Acquisition'
            };
        });
    };

    const buildTradeDataset = (ledger) => ({
        id: 'trades',
        type: 'scatter',
        label: 'Trade Bias (USDC)',
        data: ledger.map((entry) => ({
            x: entry.offsetMinutes,
            y: entry.trade.signedNotional,
            trade: entry.trade,
    ...
    const buildBalanceDataset = (ledger) => {
        const points = ledger.map((entry) => ({
            x: entry.offsetMinutes,
            y: entry.balance,
            trade: entry.trade,
            balance: entry.balance,
        ...
        points.unshift({ x: 0, y: 0, baseline: true });
        return {
            id: 'bias-balance',
            type: 'line',
            label: 'Net Bias Balance (USDC)',
            data: points,
            parsing: false,
            spanGaps: true,
            tension: 0.18,
    ...
    const buildLedgerCard = (windowData, ledger) => {
        const card = document.createElement('section');
        card.className = 'ledger-card';

        const title = document.createElement('h3');
        title.textContent = 'Redemptions & Held Trades';
        card.appendChild(title);

        if (!ledger.length) {
            const empty = document.createElement('p');
            empty.className = 'empty-state';
            empty.textContent = 'No trade ledger entries captured for this window yet.';
            card.appendChild(empty);
            return card;
        }

        const redemptionCount = ledger.filter((entry) => entry.activity === 'Redemption').length;
        const acquisitionCount = ledger.length - redemptionCount;
        const finalBalance = ledger[ledger.length - 1].balance;
        const totalCost = Number(windowData.totalCostUSDC ?? 0);
        const totalPayout = Number(windowData.totalPayoutUSDC ?? 0);
        const netProfit = Number(windowData.netProfitUSDC ?? 0);
        const postCloseRedemption = Number(windowData.postCloseRedemptionUSDC ?? 0);
        const postCloseCount = Number(windowData.postCloseRedemptionCount ?? 0);
        const roiValue = Number(windowData.roiPct);
        const hasRoi = totalCost > 0 && Number.isFinite(roiValue);

        const summary = document.createElement('p');
        summary.textContent = `${redemptionCount} redemption${redemptionCount === 1 ? '' : 's'} · ${acquisitionCount} acquisition${acquisitionCount === 1 ? '' : 's'} · Net bias balance ${formatSignedUsd(finalBalance)}`;
        card.appendChild(summary);

        const profitability = document.createElement('p');
        profitability.textContent = `Spent ${formatUsd(totalCost)} · Returned ${formatUsd(totalPayout)} · Net ${formatSignedUsd(netProfit)}${hasRoi ? ` (ROI ${formatPct(roiValue)})` : ''}`;
        card.appendChild(profitability);

        if (postCloseCount > 0) {
            const redemptionSummary = document.createElement('p');
            redemptionSummary.textContent = `${postCloseCount} post-close redemption${postCloseCount === 1 ? '' : 's'} totaling ${formatUsd(postCloseRedemption)}.`;
            card.appendChild(redemptionSummary);
        }
        ...
        ledger.forEach((entry) => {
            const { trade, balance, activity } = entry;
            const row = document.createElement('tr');
            row.classList.add(activity === 'Redemption' ? 'redemption' : 'acquisition');

            const cells = [
                activity,
                trade.outcome || 'Unknown',
                trade.side || 'UNKNOWN',
                trade.etLabel || formatEtTimestampMs(trade.tsMs),
                quantityFmt.format(trade.size),
                quantityFmt.format(trade.price),
                formatUsd(trade.notional),
                formatSignedUsd(trade.signedNotional),
                formatSignedUsd(balance)
            ];
        ...
        const applyFilters = () => {
            const filters = filterInputs.map((input) => input.value.trim().toLowerCase());
            Array.from(tbody.rows).forEach((row) => {
                const cells = Array.from(row.cells);
                const matches = filters.every((filter, idx) => {
                    if (!filter) return true;
                    return cells[idx].textContent.toLowerCase().includes(filter);
                });
    ...
    if (generatedAtEl && PAYLOAD.generatedAt) generatedAtEl.textContent = new Date(PAYLOAD.generatedAt).toLocaleString();
    ...
    const renderCard = (windowData, index, total) => {
        const ledger = buildLedger(windowData);

        const layout = document.createElement('div');
        layout.className = 'chart-grid';

        const chartColumn = document.createElement('div');
        chartColumn.className = 'chart-grid-column chart-grid-column--main';
        const ledgerColumn = document.createElement('div');
        ledgerColumn.className = 'chart-grid-column chart-grid-column--aside';
        ...
        pnlSpan.textContent = `PnL: ${formatSignedUsd(windowData.netProfitUSDC)}`;
        meta.appendChild(pnlSpan);

        const spendSpan = document.createElement('span');
        spendSpan.textContent = `Spent ${formatUsd(windowData.totalCostUSDC)}`;
        meta.appendChild(spendSpan);

        const returnSpan = document.createElement('span');
        returnSpan.textContent = `Returned ${formatUsd(windowData.totalPayoutUSDC)}`;
        meta.appendChild(returnSpan);

        const roiSpan = document.createElement('span');
        roiSpan.textContent = `ROI ${formatPct(windowData.roiPct)}`;
        meta.appendChild(roiSpan);

        if (Number(windowData.postCloseRedemptionCount) > 0) {
            const redemptionSpan = document.createElement('span');
            redemptionSpan.textContent = `${windowData.postCloseRedemptionCount} post-close redemption${windowData.postCloseRedemptionCount === 1 ? '' : 's'} (${formatUsd(windowData.postCloseRedemptionUSDC)})`;
            meta.appendChild(redemptionSpan);
        }
        ...
        if (!ledger.length) {
            ...
            chartColumn.appendChild(card);
            ledgerColumn.appendChild(buildLedgerCard(windowData, ledger));
            layout.appendChild(chartColumn);
            layout.appendChild(ledgerColumn);
        ...
        const tradeDataset = buildTradeDataset(ledger);
        const balanceDataset = buildBalanceDataset(ledger);
        const priceDataset = buildPriceDataset(windowData);

        const datasets = [priceDataset, balanceDataset, tradeDataset].filter((dataset) => dataset.data && dataset.data.length);

        const biasValues = [
            ...balanceDataset.data.map((point) => point.y),
            ...tradeDataset.data.map((point) => point.y)
        ];
        const biasRange = computeSymmetricRange(biasValues);
        const priceValues = priceDataset.data ? priceDataset.data.map((point) => point.y) : [];
        ...
        const chart = typeof Chart === 'undefined' ? null : new Chart(canvas.getContext('2d'), {
            type: 'scatter',
            data: { datasets },
            options: {
                responsive: true,
                ...
                scales: {
                    ...
                    y1: {
                        display: Boolean(priceDataset.data && priceDataset.data.length),
                        position: 'right',
                        title: { display: Boolean(priceDataset.data && priceDataset.data.length), text: 'BTC Price (USD)' },
                        ...
                        callbacks: {
                            label: (ctx) => {
                                if (ctx.dataset.id === 'trades') {
                                    ...
                                    return [
                                        'Trade: ' + (ctx.raw.activity || 'Trade'),
                                        'Time: ' + (trade.etLabel || formatEtTimestampMs(trade.tsMs)),
                                        'Outcome: ' + trade.outcome + ' (' + trade.side + ')',
                                        'Signed Bias: ' + formatSignedUsd(trade.signedNotional),
                                        'Net Balance: ' + formatSignedUsd(ctx.raw.balance),
                                        'Notional: ' + formatUsd(trade.notional),
                                        'Size: ' + quantityFmt.format(trade.size),
                                        'Price: ' + quantityFmt.format(trade.price)
                                    ];
                                }
                                if (ctx.dataset.id === 'bias-balance') {
                                    if (ctx.raw.baseline) return 'Opening balance: $0';
                                    return [
                                        'Net Balance: ' + formatSignedUsd(ctx.raw.balance),
                                        'At: ' + formatEtTimestampMs(ctx.raw.trade.tsMs)
                                    ];
                                }
    ...
    const showFrame = (index) => {
        ...
        chartContainer.appendChild(rendered.layout);
...
function buildClientScript() {
    return `(${clientBootstrap.toString()})();`;
}
...
function writeHtml(summaries, filePath) {
    ...
    const payload = {
        ...
        windows: summaries.map(summary => ({
            ...
            totalCostUSDC: summary.totalCost,
            totalPayoutUSDC: summary.totalPayout,
            postCloseRedemptionUSDC: summary.postCloseRedemptionNotional,
            postCloseRedemptionCount: summary.postCloseRedemptionCount,
            netProfitUSDC: summary.netProfit,
...
<style>
    ...
    .chart-grid-column--main { flex: 3; }
    .chart-grid-column--aside { flex: 2; max-width: 480px; }
    .chart-card {
    ...
    .ledger-table tbody tr.redemption {
        color: #fca5a5;
    }
    .ledger-table tbody tr.acquisition {
        color: #bbf7d0;
    }
    ...
    @media (max-width: 940px) {
        ...
        .chart-grid-column--aside { max-width: none; }
    }
...
async function main() {
    try {
        const grouped = await fetchTradesLive();
